...
#include "garagejam.h"
#include "garagejam-gingerblue-studio-player.h"
#include "garagejam-gingerblue-studio-player-kb.h"
...
void garagejam_studio_player_main(gchar *streamuri, gchar *name)
{
	...
	/* } */
	/* else { */
	/*   g_message ("Specify the media file name..<garagejam_studio-playerplayer filename>"); */
	/*   return 0; */
	/* } */

	Gingerblue->player_loop = g_main_loop_new(NULL, FALSE);
	g_main_loop_run(Gingerblue->player_loop);
	gst_deinit();
	Gingerblue->player_status = GINGERBLUE_STUDIO_PLAYER_TRUE;
	exit(0);
}

#if 0
static void end_of_stream_cb(GstPlayer *player, GstPlay *play)
{
	g_print("\n");
...
#endif
#if 0
static void error_cb(GstPlayer *player, GError *err, GstPlay *play)
{
...
position_updated_cb(GstPlayer *player, GstClockTime pos, GstPlay *play)
{
	...
	if (pos != -1 && dur > 0 && dur != -1) {
		...
		g_snprintf(pstr, 32, "%" GST_TIME_FORMAT,
			   GST_TIME_ARGS(pos));
		pstr[9] = '\0';
		g_snprintf(dstr, 32, "%" GST_TIME_FORMAT,
			   GST_TIME_ARGS(dur));
		dstr[9] = '\0';
...
static void buffering_cb(GstPlayer *player, gint percent, GstPlay *play)
{
	g_print("Buffering: %d\n", percent);
}
...
static void print_all_stream_info(GstPlayerMediaInfo *media_info)
{
	guint count = 0;
	GList *list, *l;
...
static void print_all_video_stream(GstPlayerMediaInfo *media_info)
{
	GList *list, *l;
...
static void print_all_subtitle_stream(GstPlayerMediaInfo *media_info)
{
	GList *list, *l;
...
static void print_all_audio_stream(GstPlayerMediaInfo *media_info)
{
	GList *list, *l;
...
static void print_current_tracks(GstPlay *play)
{
...
static void print_media_info(GstPlayerMediaInfo *media_info)
{
...
#if 0
static GstPlay *play_new(gchar **uris, gdouble initial_volume)
{
	GstPlay *play;
...
static void play_set_relative_volume(GstPlay *play, gdouble volume_step)
{
	...
	volume =
	    round((volume + volume_step) * VOLUME_STEPS) / VOLUME_STEPS;
...
#if 0
static gchar *play_uri_get_display_name(GstPlayer *player,
					const gchar *uri)
{
	gchar *loc;
...
static void do_play(GstPlay *play)
{
	gint i;
	/* dump playlist */
...
static void add_to_playlist(GPtrArray *playlist, const gchar *filename)
{
	...
	if ((dir = g_dir_open(filename, 0, NULL))) {
		...
		while ((entry = g_dir_read_name(dir))) {
			gchar *path;
			path =
			    g_strconcat(filename, G_DIR_SEPARATOR_S, entry,
					NULL);
...
static void shuffle_uris(gchar **uris, guint num)
{
	gchar *tmp;
	guint i, j;
	if (num < 2)
...
int main(int argc, char **argv)
{
	...
	ctx =
	    ...
	    ("FILE1|URI1 [FILE2|URI2] [FILE3|URI3] ...");
	...
	if (playlist_file != NULL) {
		...
		if (g_file_get_contents
		    (playlist_file, &playlist_contents, NULL, &err)) {
			lines = g_strsplit(playlist_contents, "\n", 0);
			num = g_strv_length(lines);
			for (i = 0; i < num; i++) {
				if (lines[i][0] != '\0') {
					GST_LOG("Playlist[%d]: %s", i + 1,
						lines[i]);
...
#endif
void garagejam_studio_player_play(GtkWidget *widget,
				  gpointer *recording_entry)
{
	garagejam_studio_player_stop(player);
	player =
	    gst_player_new(NULL,
			   gst_player_g_main_context_signal_dispatcher_new
			   (NULL));
